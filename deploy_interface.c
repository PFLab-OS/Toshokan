...
static ssize_t
deploy_bin_write(struct file *filp, struct kobject *kobj,
...
int __init deploy_interface_init(void)
{
  ...
  deploy_sysfs_kobj = kobject_create_and_add("deploy", &THIS_MODULE->mkobj.kobj);
...
void __exit deploy_interface_exit(void)
{
    kobject_put(deploy_sysfs_kobj);
}
