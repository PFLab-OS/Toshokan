...
QEMU上からメモリを確認してみましょう。`make`を実行させたターミナルとは別のターミナルで`make monitor`を実行した後、QEMUモニタで`x /2x 0x80000000`としてください。
...
実は、先程のデータと一致するデータは、物理メモリ上では0x100000000に存在します。

```
(qemu) xp /2x 0x100000000
0000000100000000: 0x464c457f 0x00010102
```
...
friend.cc上で、仮想メモリ0x80000000からの4KBと物理メモリ0x100000000からの4KBを対応付けるコードが以下の通りです。コード内で、vaddrと0x100000000が参照されていますね。
