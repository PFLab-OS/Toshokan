...
docker_cmd = 'docker run --rm -v {0}:{0} '.format(curdir)
...
Command('hakase/FriendLoader/trampoline/bin.o', 'hakase/FriendLoader/trampoline/boot_trampoline.bin', [
          docker_module_build_cmd + 'sh -c "cd {0}; objcopy -I binary -O elf64-x86-64 -B i386:x86-64 $SOURCES $TARGET"'.format(curdir),
	  docker_module_build_cmd + 'sh -c "cd {0}; script/check_trampoline_bin_size.sh $TARGET"'.format(curdir)])
Command('hakase/FriendLoader/friend_loader.ko', [Glob('hakase/FriendLoader/*.h'), Glob('hakase/FriendLoader/*.c'), 'hakase/FriendLoader/trampoline/bin.o'], docker_module_build_cmd + 'sh -c "cd {0}; KERN_VERSION=4.13.0-45-generic make all"'.format(curdir + '/hakase/FriendLoader'))
