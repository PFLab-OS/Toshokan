...
int test_main(F2H2 &f2h, H2F2 &h2f, I2H2 &i2h, int argc, const char **argv) {
  ...
  CallerChannelAccessor caller_ca(h2f, Channel2::Id(1), Channel2::Signal::kExec());
  caller_ca.Write<uint64_t>(CallerChannelAccessor::Offset<uint64_t>(0), kDeployAddressStart);
  if (caller_ca.Call() != 0) {
    return 1;
  }
