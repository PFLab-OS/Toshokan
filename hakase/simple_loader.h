...
class SimpleLoader {
 public:
  struct DeployData {
    DeployData(uint64_t offset, size_t size, uint8_t *buf)
        : _offset(offset), _size(size), _buf(buf) {}
    DeployData() = delete;
    ~DeployData() { delete _buf; }
    uint64_t _offset;
    size_t _size;
    uint8_t *_buf;
  };
  ...
  class BinaryFile {
   public:
    ...
    std::unique_ptr<DeployData> GetData() {
      ...
      std::unique_ptr<DeployData> dd(
          new DeployData(_offset, (*_it).first, (*_it).second));
      _offset += (*_it).first;
      _it++;
      return dd;
    }
