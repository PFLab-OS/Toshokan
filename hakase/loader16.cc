...
int Loader16::Init(uint64_t entry) {
   ...
   */
  char *bootmem =
      static_cast<char *>(mmap(NULL, TRAMPOLINE_SIZE, PROT_READ | PROT_WRITE,
                               MAP_SHARED, bootmem_fd, 0));
...
void Loader16::InitPageTables(char *pml4t_addr) {
  ...
  static const uint64_t kHugePageTableEntryFlags =
      (1 << 0) | (1 << 1) | (1 << 2) | (1 << 7);

  static_assert((TRAMPOLINE_ADDR % k256TB) / k512GB ==
                    (DEPLOY_PHYS_ADDR_START % k256TB) / k512GB,
                "");

  pml4t->entry[(DEPLOY_PHYS_ADDR_START % k256TB) / k512GB] =
      reinterpret_cast<size_t>(&SHARED_SYMBOL(__toshokan_pdpt)) |
      kPageTableEntryFlags;
  SHARED_SYMBOL(__toshokan_pdpt).entry[(TRAMPOLINE_ADDR % k512GB) / k1GB] =
      reinterpret_cast<size_t>(&SHARED_SYMBOL(__toshokan_pd1)) |
      kPageTableEntryFlags;
  SHARED_SYMBOL(__toshokan_pdpt)
      .entry[(DEPLOY_PHYS_ADDR_START % k512GB) / k1GB] =
      reinterpret_cast<size_t>(&SHARED_SYMBOL(__toshokan_pd2)) |
      kPageTableEntryFlags;
